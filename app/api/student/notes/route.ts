...
// GET /api/student/notes - Get notes for the current student
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all notes and filter for this student
    ...
    // Filter notes that are sent to this student
    const studentNotes = allNotes.filter((note: any) => {
      try {
        const recipientIds = JSON.parse(note.recipientIds || '[]')
        return recipientIds.includes(userId) || 
               note.recipientType === 'all' || 
               (note.recipientType === 'students' && session.user.role === 'student')
      } catch (e) {
        return false
      }
    })

    // Format the notes for the student
