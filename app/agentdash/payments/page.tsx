...
export default function AgentPaymentsPage() {
  ...
  const handleCreateCommission = async () => {
    try {
      ...
      const autoGeneratedOrderId = Math.floor(Math.random() * 10000) + 1000;
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedCommission && (
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>إيصال العمولة</CardTitle>
                </CardHeader>
                <CardContent>
                  {selectedCommission.receiptPath ? (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                      ...
                      <div className="space-y-2 text-sm text-green-700 mb-4">
                        <div>
                          <strong>تاريخ الرفع:</strong> {selectedCommission.receiptUploadedAt ? new Date(selectedCommission.receiptUploadedAt).toLocaleDateString('ar-SA') : 'غير محدد'}
                        </div>
                        <div>
                          <strong>حالة التسليم:</strong> {selectedCommission.deliveredToAgent ? 'تم التسليم' : 'لم يتم التسليم بعد'}
                        </div>
                        {selectedCommission.deliveredAt && (
                          <div>
                            <strong>تاريخ التسليم:</strong> {new Date(selectedCommission.deliveredAt).toLocaleDateString('ar-SA')}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex gap-2">
                        <Button 
                          ...
                          size="sm"
                          onClick={() => {
                            window.open(selectedCommission.receiptPath!, '_blank');
                            // Mark as viewed by agent
                            fetch(`/api/agent/commissions/${selectedCommission.id}/mark-viewed`, {
                              method: 'POST'
                            });
                          }}
                        >
                    ...
                    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        سيتم إشعارك عند رفع الإيصال من قبل الإدارة
                      </p>
                    </div>
                  )}
                </CardContent>
